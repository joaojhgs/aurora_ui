    ...
    def __init__(self, message, is_user=False, parent=None, dark_mode=False, source_type=None):
        ...
        if self.dark_mode:
            timestamp.setStyleSheet(
                "color: #AAAAAA; font-size: 10px; background-color: transparent;")
        else:
            timestamp.setStyleSheet(
                "color: #777777; font-size: 10px; background-color: transparent;")
        ...
        if not is_user:
            has_markdown = (
                '```' in message_str  # code blocks
                or re.search(r'\*\*.+\*\*', message_str)  # bold
                or re.search(r'\*.+\*', message_str)  # italic
                or re.search(r'#+\s', message_str)  # headers
                or re.search(r'^\s*[-*+]\s', message_str, re.MULTILINE)  # list items
                or '|' in message_str  # potential tables
                or '[' in message_str and '](' in message_str  # links
            )
...
"""
    ...
    def _add_message_to_ui_only(self, message, is_user=False, source_type=None):
        ...
        log_debug(
            f"UI: Adding message to UI only: '{message[:30]}...' User: {is_user} Source: {source_type}")
    ...
    def init_ui(self):
        ...
        if self.dark_mode:
            bottom_panel.setStyleSheet(
                "background-color: #1E1E1E; border-top: 1px solid #333333; padding: 10px;")
        else:
            bottom_panel.setStyleSheet(
                "background-color: #F5F5F5; border-top: 1px solid #E0E0E0; padding: 10px;")
        ...
        class EnterTextEdit(QTextEdit):
            ...
            def keyPressEvent(self, event):
                ...
                if (event.key() == Qt.Key.Key_Return or event.key()
                        == Qt.Key.Key_Enter) and not event.modifiers() & Qt.KeyboardModifier.ShiftModifier:
                    self.send_callback()
                    return
        ...
        self.input_field.setPlaceholderText(
            "Type your message here (press Enter to send, Shift+Enter for newline)...")
        self.input_field.setMaximumHeight(80)
        ...
        if self.dark_mode:
            self.stop_button.setStyleSheet(button_style %
                                           ('#333333', '#E0E0E0', '#444444', '#222222'))
            self.dark_mode_button.setStyleSheet(
                button_style %
                ('#333333', '#E0E0E0', '#444444', '#222222'))
        else:
            self.stop_button.setStyleSheet(button_style %
                                           ('#E0E0E0', '#333333', '#CCCCCC', '#BBBBBB'))
            self.dark_mode_button.setStyleSheet(
                button_style %
                ('#E0E0E0', '#333333', '#CCCCCC', '#BBBBBB'))
    ...
    def create_menu_bar(self):
        ...
        export_action.setStatusTip('Export chat history to file')
        ...
        dark_mode_action.setStatusTip('Toggle dark mode theme')
        ...
        about_action.setStatusTip('About Aurora AI Assistant')
    ...
    def open_configuration(self):
        ...
        try:
            from .config_modal import ConfigurationModal

            dialog = ConfigurationModal(self)
            result = dialog.exec()

            if result == dialog.DialogCode.Accepted:
                # Configuration was changed, might need UI updates
                from app.config.config_manager import config_manager
                new_dark_mode = config_manager.get('ui.dark_mode', False)
                ...
                if new_dark_mode != self.dark_mode:
                    ...
                    self._refresh_chat_styling()

        except Exception as e:
            from app.helpers.aurora_logger import log_error
            log_error(f"Error opening configuration dialog: {e}")
    ...
    def export_chat(self):
        ...
        try:
            ...
            messages = self.message_history.get_today_messages()
            ...
            if file_path.endswith('.json'):
                ...
                # Export as text
                with open(file_path, 'w', encoding='utf-8') as f:
                    f.write(f"Aurora AI Assistant Chat Export\n")
                    f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                    f.write(f"Version: {self.version}\n")
                    f.write("=" * 50 + "\n\n")

                    for msg in messages:
                        prefix = "User" if msg.is_user_message() else "Aurora"
                        timestamp = msg.timestamp.strftime('%H:%M:%S')
                        f.write(f"[{timestamp}] {prefix}: {msg.content}\n\n")
    ...
    def clear_chat(self):
        ...
        reply = QMessageBox.question(
            ...
            "This will only clear the display. Database history will be preserved.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No
        )

        if reply == QMessageBox.StandardButton.Yes:
            # Clear all message widgets except welcome message
    ...
    def show_about(self):
        ...
        about_text = f"""
<h3>Aurora AI Assistant v{self.version}</h3>
<p>A modern voice and text AI assistant with advanced features.</p>

<h4>Features:</h4>
<ul>
<li>Voice commands with Speech-to-Text</li>
<li>Text input with detailed responses</li>
...
<p><small>Built with ❤️ for advanced AI interactions</small></p>
        """

        QMessageBox.about(self, "About Aurora", about_text)

    def _refresh_chat_styling(self):
        """Refresh the styling of existing chat messages when theme changes"""
        ...
        # Refresh existing messages by recreating them
    ...
    def add_message(self, message, is_user=False, source_type=None):
        ...
        # Debug logging
        log_debug(
            f"UI: Adding message to chat: '{message[:30]}...' User: {is_user} Source: {source_type}")
    ...
    def _recreate_message_widgets(self):
        ...
        # Remove all existing message widgets except the stretch
        while self.chat_layout.count() > 1:  # Keep the stretch at the end
            child = self.chat_layout.takeAt(0)
            if child.widget():
                child.widget().deleteLater()

        # Recreate widgets with updated styling
        for msg_data in messages_data:
            new_widget = MessageWidget(
                msg_data['content'],
                msg_data['is_user'],
                dark_mode=self.dark_mode,
                source_type=msg_data['source_type']
            )
            self.chat_layout.insertWidget(self.chat_layout.count() - 1, new_widget)
    ...
    def hook_into_systems(self):
        ...
        tts.stream = TextToAudioStream(
            tts.engine,
            frames_per_buffer=256,
            on_audio_stream_start=ui_on_audio_stream_start,
            on_audio_stream_stop=ui_on_audio_stream_stop)
